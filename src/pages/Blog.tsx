...
const Blog = () => {
  ...
  // Move extractTagsFromPost above useMemo to avoid ReferenceError
  const extractTagsFromPost = (post: SanityPost) => {
      ...
      post.body
        ?.map((block: unknown) =>
          typeof block === "object" && block !== null && "children" in block
            ? (block as { children?: BlockChild[] }).children
    ...
    destinations.forEach((dest) => {
      if (
        titleWords.some((word) => dest.toLowerCase().includes(word)) ||
    ...
    types.forEach((type) => {
      if (
        titleWords.some((word) => type.toLowerCase().includes(word)) ||
  ...
  const getExcerpt = (body: BodyBlock[]) => {
    const firstTextBlock = body?.find(
      (block) => block._type === "block" && block.children?.[0]?.text
    );
    return (
  ...
  return (
    <>
      <Helmet>
        <title>
          Travel Stories & Adventure Insights - Wilderness Alternative
          Expedition
        </title>
        <meta
          name="description"
          content="Discover travel stories, trekking guides, and safari insights from Kenya's wilderness. Expert tips for Mount Kenya, Kilimanjaro, and African safaris."
        />
        <meta
          name="keywords"
          content="Kenya travel blog, Mount Kenya trekking guides, Kilimanjaro climbing tips, safari stories, Africa adventure blog"
        />
        <meta
          property="og:title"
          content="Travel Stories & Adventure Insights"
        />
        <meta
          property="og:description"
          content="Tips, guides, and stories from Kenya's wild adventures"
        />
        ...
        <section className="relative py-20 bg-gradient-adventure text-white">
          <div className="container mx-auto px-4 text-center">
            ...
            <p className="text-xl md:text-2xl text-white/90 max-w-3xl mx-auto">
              Tips, guides, and stories from the wild. Discover expert insights
              from our adventures across Kenya's mountains and savannas.
            </p>
          </div>
        </section>
              ...
              {loading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <details className="mb-6">
                    ...
                    <pre className="bg-muted p-2 rounded text-xs text-left max-w-xl mx-auto overflow-x-auto">
                      {typeof error === "string"
                        ? error
                        : JSON.stringify(
                            error,
                ...
                (() => {
                  console.log("Sanity posts:", filteredPosts);
                  return (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                      {filteredPosts.map((post) => (
                        <Card
                          key={post?._id || Math.random()}
                          className="overflow-hidden hover:shadow-adventure transition-all duration-300 group cursor-pointer"
                        >
